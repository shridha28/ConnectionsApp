...
public class CustomerServiceImplTest {
	...
	/*
	 * @Test void create_Customer_Shud_CreateAUserWithRole() throws Exception{ Role
	 * role = new Role(1001,"user", null, null); Set<Role> roles = new HashSet<>();
	 * roles.add(role); CustomerEntity cust = new
	 * CustomerEntity(null,"abc@gmail.com", "ddd",null, roles,null,null);
	 * 
	 * 
	 * Mockito.when(repo.save(cust)).thenReturn(cust); CustomerEntity customer
	 * =serviceImpl.saveCustomer(cust); Assert.assertEquals(cust , customer); }
	 */
	...
	/*
	 * @Test void get_customerByEmailId_test() throws Exception{ String emailId =
	 * "abc@gmail.com"; AddressEntity adress = new
	 * AddressEntity("street","11-6","mark the land","city","state"); CustomerEntity
	 * cust = new CustomerEntity(null,"abc@gmail.com","ddd","abc", null,
	 * null,adress); Mockito.when(repo.findByEmailid(emailId)).thenReturn(cust);
