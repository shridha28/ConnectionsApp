package com.myproject.connections.test.mappers;
 ...
 *Test  class to check functionality for StatesMapper class 
 */
public class StatesMapperTest {
	...
	private static Logger logger = LoggerFactory.getLogger(StatesMapperTest.class);
	 ...
	 /*
	  * method to test the mapping of StatesDto object to StatesEntity object
	  */
	@Test
	public void convertStatesDtoToStatesEntity_test()

	{
		...
		List<CityDto> citiesDto = Arrays.asList(new CityDto("KA", "Bangalore"), new CityDto("KA", "Mysore"));
		...
		logger.info(
				"Confirming if the list of cityDto Objects in StateDto is mapped to list of CityEntity Objects in State Entity Object");
	...
	/*
	  * method to test the mapping of StatesEntity object to StatesDto object
	  */
	@Test
	public void convertStatesEntityToStatesDto_test()

	{
		logger.info("Creating a Mock Object for StatesEntity");
		StatesEntity statesEntity1 = new StatesEntity();
		statesEntity1.setStateName("Karnataka");
		...
		@SuppressWarnings("deprecation")
		Date modifiedDate = new Date("12/12/1990");
		statesEntity1.setModified_date(modifiedDate);
		List<CityEntity> citiesEntity = Arrays.asList(new CityEntity("KA", "Bangalore"),
				new CityEntity("KA", "Mysore"));
		statesEntity1.setCities(citiesEntity);
		logger.info("Mapping StatesEntity Object to StatesDto Object using ModelMapper");
		...
		logger.info(
				"Confirming if the list of cityEntity1 Objects in StateEntity1 is mapped to list of CityDto1 Objects in State Dto Object");
		assertNotEquals(statesEntity1.getCities(), statesDto1.getCities());
		assertEquals(statesEntity1.getCities().get(0).getCityName(), statesDto1.getCities().get(0).getCityName());
		assertEquals(statesEntity1.getCities().get(1).getCityName(), statesDto1.getCities().get(1).getCityName());
		assertEquals(statesEntity1.getCities().get(0).getCstateID(), statesDto1.getCities().get(0).getCstateID());
		assertEquals(statesEntity1.getCities().get(1).getCstateID(), statesDto1.getCities().get(1).getCstateID());
	}
